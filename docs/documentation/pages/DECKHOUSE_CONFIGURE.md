...
- `spec.version` — a version of module settings. Latest version number is available in the module's documentation.
- `spec.settings` — module settings.
- `spec.enabled` — boolean value to explicitly [enable or disable the module](#enabling-and-disabling-the-module).

`ModuleConfig/global` stores global settings, `enabled` parameter is ignored for this resource.
