...
var _ = Describe("Module :: deckhouse-config :: hooks :: update DeckhouseConfig status ::", func() {
	...
	Context("Known module enabled", func() {
		...
		It("Should be enabled in status", func() {
			...
			promCfg := f.KubernetesGlobalResource("DeckhouseConfig", "module-one")
	...
	Context("Enabled by bundle, disabled by enabled script", func() {
		...
		It("Should not be enabled in status", func() {
			...
			promCfg := f.KubernetesGlobalResource("DeckhouseConfig", "module-one")
			Expect(promCfg.Field("status.enabled").String()).To(ContainSubstring("Disabled by script"), "should be disabled by script, got %s", promCfg.Field("status").String())
		})
	})
})
